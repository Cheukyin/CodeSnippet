...
int withinSameBlock(int * ptr1, int * ptr2) {
  // TODO
    unsigned int u_ptr1 = ptr1;
    unsigned int u_ptr2 = ptr2;
    //64-byte = 2^6, mask = 0b11...11 000 0000
    unsigned int mask = (1<<31) >> (32-7);

    //找出离u_ptr最近且比u_ptr小的2^6的整倍数
    //check if they are the same
    return (u_ptr1 & mask) == (u_ptr2 & mask);
}
...
int withinArray(int * intArray, int size, int * ptr) {
  // TODO
    //keypoint: check if ptr == 4*(size-1) + intArray
    int i_array = intArray;
    int i_ptr = ptr;

    //return (size-1)*4 == (ptr - intArray)
    return ( (size+~0)<<2 ) == (i_ptr - i_array);
}
/*
