...
void eval(char *cmdline)
{
    ...
    if( !builtin_cmd(argv) )
    {
        // block SIGCHLD to add child to joblist
        signal_mask(SIG_BLOCK, SIGCHLD);

        if( !(pid = fork()) ) // child
        {
            // child will inherit the block signals,
            // so the child should unblock SIGCHLD
            signal_mask(SIG_UNBLOCK, SIGCHLD);

            // put the child into a new pgrp to
            // ensure the child's not in the same grp of tsh
            if(setpgid(0, 0) < 0)
